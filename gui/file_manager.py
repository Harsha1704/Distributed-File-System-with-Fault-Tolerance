...
class FileManager(QWidget):
    def __init__(self):
        ...
        h_layout.addWidget(badge, alignment=Qt.AlignLeft)
        ...
        metrics.addWidget(self.files_card)
        metrics.addWidget(self.types_card)
        metrics.addWidget(self.chunks_card)
        ...
        self.search_box.setPlaceholderText('Search files by name...')
        ...
        t_layout.addWidget(self.search_box, 1)
        t_layout.addWidget(self.upload_btn)
        t_layout.addWidget(self.download_btn)
        t_layout.addWidget(self.delete_btn)
        ...
        self.d_desc = QLabel('Select a file from the table to inspect its metadata. The old “Selection preview” button was removed and replaced with this details panel.')
        self.d_desc.setObjectName('smallMuted'); self.d_desc.setWordWrap(True)
        d_layout.addWidget(d_title)
        d_layout.addWidget(self.d_name)
        d_layout.addWidget(self.d_type)
        d_layout.addWidget(self.d_size)
        d_layout.addWidget(self.d_desc)
    ...
    def update_selection_details(self):
        ...
        if not filename:
            ...
            self.d_name.setText('No file selected'); self.d_type.setText('Type: --'); self.d_size.setText('Size / Chunks: --')
            self.d_desc.setText('Select a file from the table to inspect its metadata. The old “Selection preview” button was removed and replaced with this details panel.')
            GUI_STATE.update_stats(selected_file='--', selected_type='--', selected_size='--')
            return
        ...
        self.d_name.setText(filename); self.d_type.setText(f'Type: {ftype}'); self.d_size.setText(f'Size / Chunks: {ssize}')
        self.d_desc.setText(f'Selected file is stored in the distributed system metadata as {filename}.')
        GUI_STATE.update_stats(selected_file=filename, selected_type=ftype, selected_size=ssize)
    ...
    def refresh_files(self):
        try:
            ...
            total_chunks = sum(f.get('num_chunks', 0) for f in self.all_files)
            ...
            self.apply_filter()
            GUI_STATE.update_stats(total_files=len(self.all_files), total_chunks=total_chunks, file_types=len(file_types))
