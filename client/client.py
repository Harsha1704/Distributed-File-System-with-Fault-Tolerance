...
def _master_conn() -> socket.socket:
    try:
        ...
        logger.error(f"Cannot connect to master ({MASTER_HOST}:{MASTER_PORT}) → {e}")
        sys.exit(1)
...
def upload(local_path: str) -> None:
    ...
    with _master_conn() as sock:
        send_message(sock, {
            ...
            "file_hash":  file_hash,
            "chunk_meta": [
                {"chunk_id": c["chunk_id"], "size": c["size"], "hash": c["hash"]}
                for c in chunks
            ],
        })
...
def download(remote_filename: str) -> None:
    ...
    logger.info(f"Download plan for '{remote_filename}': {len(chunks)} chunk(s)")
    ...
    if len(chunks) == 0:
        ...
        print(f"\n[OK] Downloaded EMPTY file → {output}")
        return
